...
def summary(scen_name: str, print_summary: bool = True) -> Optional[str]:
    ...
    # Read params.mat and provide TXRX summary, total number of tx & rx, 
    # scene size, and other relevant parameters, computed/extracted from 
    # all dicts, not just rt_params
    ...
    try:
        stats_str = stats_summary(scen_name, print_summary)
        summary_str += "\n[Scenario Statistics]\n"
        summary_str += stats_str
    except Exception as e:
        print(f"Error calculating scenario statistics: {str(e)}\n")
        print("Note: Detailed statistics require loading the complete dataset.\n")
...
def stats_summary(scen_name: str, print_summary: bool = False, bs_idx: int = 0) -> str:
    """
    Calculate and return a summary of the scenario statistics.
    
    Args:
        scen_name (str): Name of the scenario to summarize
        print_summary (bool): Whether to print the summary (default: False)
        bs_idx (int): Base station index for statistics (default: 0, first base station)

    Returns:
        str: Summary string if print_summary is False, None otherwise
    """

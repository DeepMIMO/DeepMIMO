...
FUNDAMENTAL_MATRICES_HELP = {
    # Power and phase
    c.POWER_PARAM_NAME: "Tap power. Received power in dBW for each path, assuming 0 dBW transmitted power. \n"
    "10*log10(|a|²), where a is the complex channel amplitude\n"
    "\t[num_rx, num_paths]",
    c.PHASE_PARAM_NAME: "Tap phase. Phase of received signal for each path in degrees. \n"
    ...
    "\t[num_rx, num_paths]",
    # Interactions
    c.INTERACTIONS_PARAM_NAME: "Type of interactions along each path\n"
    "\tCodes: 0: LOS, 1: Reflection, 2: Diffraction, 3: Scattering, 4: Transmission\n"
    "\tCode meaning: 121 -> Tx-R-D-R-Rx\n"
    "\t[num_rx, num_paths]",
    c.INTERACTIONS_POS_PARAM_NAME: "3D coordinates in meters of each interaction point along paths\n"
...
COMPUTED_MATRICES_HELP = {
    c.LOS_PARAM_NAME: "Line of sight status for each path. \n"
    ...
    "\t[num_rx, ]",
    c.CHANNEL_PARAM_NAME: "Channel matrix between TX and RX antennas\n"
    ...
    c.INTER_STR_PARAM_NAME: "Interaction string for each path.\n"
...
CHANNEL_HELP_MESSAGES = {
    ...
    c.PARAMSET_ANT_BS
    + "."
    + c.PARAMSET_ANT_SHAPE: "Antenna array dimensions [X, Y] or [X, Y, Z] elements\n"
    "\t Default: [1, 1]  |  Type: list[int]  |  Units: number of elements",
    c.PARAMSET_ANT_BS + "." + c.PARAMSET_ANT_SPACING: "Spacing between antenna elements\n"
    ...
    c.PARAMSET_ANT_BS + "." + c.PARAMSET_ANT_RAD_PAT: "Antenna element radiation pattern\n"
    ...
    # OFDM Parameters
    c.PARAMSET_OFDM: f"OFDM channel configuration parameters. Used (and needed!) only if {c.PARAMSET_FD_CH}=1. \n"
    ...
    c.PARAMSET_OFDM + "." + c.PARAMSET_OFDM_SC_SAMP: "Indices of subcarriers to generate\n"

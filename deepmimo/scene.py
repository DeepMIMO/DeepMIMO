...
class PhysicalElement:
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"PhysicalElement(name='{self.name}', id={self.object_id}, "
...
class Scene:
    ...
    def from_data(cls, base_folder: str) -> "Scene":
        ...
        try:
            ...
            print(
                f"FileNotFoundError: {base_folder}/vertices.mat or {base_folder}/objects.json not found",
            )
            vertices = np.array([])
            objects_metadata = []
            ...
            for k in subset:
                ...
                for m in subset:
                    ...
                    if intersects:
                        continue
                    angle_cost = (
                        _calculate_angle_deviation(points[prev_path[-2]], points[m], points[k])
                        if len(prev_path) > 1
                        else 0
                    )

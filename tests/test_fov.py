...
def run_correctness_tests():
    ...
    mask_orig = apply_FoV(fov=fov, theta=theta_single[0], phi=phi_single[0])
    ...
    mask_batch = apply_FoV_batch(fov=fov, theta=theta_single, phi=phi_single)
    ...
    mask_orig_limited = apply_FoV(fov=fov_limited, theta=theta_single[0], phi=phi_single[0])

    # Test batch function
    mask_batch_limited = apply_FoV_batch(fov=fov_limited, theta=theta_single, phi=phi_single)
    ...
    for i in range(n_users):
        for j in range(n_paths):
            mask_1[i, j] = apply_FoV(fov=fov_full, theta=theta[i, j], phi=phi[i, j])

    # Method 2: Using batch function
    mask_2 = apply_FoV_batch(fov=fov_full, theta=theta, phi=phi)
    ...
    for i in range(n_users):
        for j in range(n_paths):
            mask_3[i, j] = apply_FoV(fov=fov_limited, theta=theta[i, j], phi=phi[i, j])

    # Method 2: Using batch function
    mask_4 = apply_FoV_batch(fov=fov_limited, theta=theta, phi=phi)
...
def run_performance_tests(n_users=1000, n_paths=5, n_trials=10):
    ...
    for _ in range(n_trials):
        ...
        for i in range(n_users):
            for j in range(n_paths):
                mask[i, j] = apply_FoV(fov=fov, theta=theta[i, j], phi=phi[i, j])
    ...
    for _ in range(n_trials):
        start = time.perf_counter()
        mask = apply_FoV_batch(fov=fov, theta=theta, phi=phi)

...
def run_correctness_tests():
    ...
    print(
        f"sin_alpha = sin(phi - gamma) = sin({np.deg2rad(phi_single[0])} - {rotation_rad[2]}) = {np.sin(np.deg2rad(phi_single[0]) - rotation_rad[2])}",
    )
    ...
    print(
        f"sin_alpha = sin(phi - gamma) = sin({phi_rad[0]} - {rotation_batch[0, 2]}) = {np.sin(phi_rad[0] - rotation_batch[0, 2])}",
    )
...
def run_performance_tests(n_users=1000, n_paths=5, n_trials=10):
    ...
    for _ in range(n_trials):
        ...
        for i in range(n_users):
            theta_rot, phi_rot = rotate_angles(rotation=rotation_single, theta=theta[i], phi=phi[i])
    ...
    for _ in range(n_trials):
        start = time.perf_counter()
        theta_rot, phi_rot = rotate_angles_batch(rotation=rotation_single, theta=theta, phi=phi)
    ...
    for _ in range(n_trials):
        start = time.perf_counter()
        theta_rot, phi_rot = rotate_angles_batch(rotation=rotation_batch, theta=theta, phi=phi)
        times_batch.append(time.perf_counter() - start)

...
class TestMatlabRTUnits(unittest.TestCase):
    ...
    def test_normalize_signed_deg_required_values(self) -> None:
        ...
        self.assertEqual(normalize_signed_deg(0), 0.0)
        self.assertEqual(normalize_signed_deg(180), 180.0)
        self.assertEqual(normalize_signed_deg(-180), -180.0)
        self.assertEqual(normalize_signed_deg(360), 0.0)
        self.assertEqual(normalize_signed_deg(-45), -45.0)
    ...
    def test_matlab_elevation_to_deepmimo_theta_required_values(self) -> None:
        ...
        self.assertEqual(matlab_elevation_to_deepmimo_theta(0), 90.0)
        self.assertEqual(matlab_elevation_to_deepmimo_theta(90), 0.0)
        self.assertEqual(matlab_elevation_to_deepmimo_theta(-90), 180.0)
    ...
    def test_matlab_path_loss_to_power_dbw_required_values(self) -> None:
        ...
        self.assertEqual(matlab_path_loss_to_power_dbw(63.0), -63.0)
        self.assertEqual(matlab_path_loss_to_power_dbw(63.0, tx_gain_db=3.0), -60.0)
        self.assertEqual(matlab_path_loss_to_power_dbw(63.0, rx_gain_db=2.0), -61.0)
        self.assertEqual(
            matlab_path_loss_to_power_dbw(
                63.0,
                tx_power_dbw=1.0,
    ...
    def test_matlab_phase_to_deg_uses_degree_input(self) -> None:
        ...
        self.assertEqual(matlab_phase_to_deg(phase_shift_deg=269.5), 269.5)
        self.assertEqual(
            matlab_phase_to_deg(phase_shift_deg=10.0, phase_shift_rad=math.pi),
            10.0,
        )

    def test_matlab_phase_rad_to_deg_and_fallback(self) -> None:
        """Convert radian phase when degree phase is unavailable."""
        self.assertAlmostEqual(matlab_phase_rad_to_deg(math.pi / 2), 90.0)
        self.assertAlmostEqual(matlab_phase_to_deg(phase_shift_rad=math.pi), 180.0)
        self.assertAlmostEqual(
            matlab_ray_phase_to_deg({"phase_shift_rad": math.pi / 4}),
            45.0,
        )

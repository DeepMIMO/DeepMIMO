...
class TestMatlabRTConverterE2E(unittest.TestCase):
    ...
    def test_public_converter_end_to_end_workflow(self) -> None:
        ...
        self.assertEqual(result.scenario_name, self.scenario_name)
        self.assertTrue((result.scenario_path / "params.json").exists())
        self.assertNotIn("matlab.engine", sys.modules)

        macro = dm.load(self.scenario_name, max_paths=2)
        self.assertIsInstance(macro, MacroDataset)
        ...
        self.assertEqual(dataset.power.shape, (2, 2))
        self.assertEqual(dataset.inter_pos.shape, (2, 2, 1, 3))
        self.assertIn("MATLAB Ray Tracing", dm.summary(self.scenario_name, print_summary=False))
        ...
        self.assertEqual(channel.shape, (2, 1, 1, 2))
        self.assertTrue(np.isfinite(channel).all())
        self.assertGreater(np.linalg.norm(channel[0]), 0.0)
        self.assertEqual(np.linalg.norm(channel[1]), 0.0)
        ...
        self.assertEqual(ofdm_channel.shape, (2, 1, 1, 8))
        self.assertTrue(np.isfinite(ofdm_channel).all())
        ...
        self.assertEqual(bf_channel.shape, (2, 1, 8, 2))
    ...
    def test_parsed_export_input_and_overwrite_policy(self) -> None:
        ...
        self.assertTrue((result.scenario_path / "power_t000_tx000_r001.npz").exists())

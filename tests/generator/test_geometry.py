...
def test_rotate_angles_zero_rotation() -> None:
    ...
    theta_orig, phi_orig = rotate_angles(
        rotation=rotation_zero, theta=theta_single[0], phi=phi_single[0]
    )
    theta_batch, phi_batch = rotate_angles_batch(
        rotation=rotation_zero, theta=theta_single, phi=phi_single
    )
...
def test_rotate_angles_batch_equivalence() -> None:
    ...
    for i in range(n_users):
        theta_rot_1[i], phi_rot_1[i] = rotate_angles(
            rotation=rotation_single, theta=theta[i], phi=phi[i]
        )
    ...
    for i in range(n_users):
        theta_rot_3[i], phi_rot_3[i] = rotate_angles(
            rotation=rotation_per_user[i], theta=theta[i], phi=phi[i]
        )

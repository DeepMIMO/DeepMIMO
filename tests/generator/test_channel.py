import pytest
import numpy as np
from deepmimo.generator.channel import ChannelParameters, _generate_MIMO_channel, _convert_lists_to_arrays
...
def test_generate_mimo_channel_time_domain():
    ...
    channel = _generate_MIMO_channel(
        array_response, powers, delays, phases, dopplers, ofdm_params,
        times=0.0, freq_domain=False
    )
...
def test_doppler_progression():
    ...
    channel = _generate_MIMO_channel(
        array_response, powers, delays, phases, dopplers, ofdm_params,
        times=times, freq_domain=False, squeeze_time=False
    )

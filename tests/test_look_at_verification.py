...
def test_basic_functionality():
    ...
    print("Expected: azimuth≈0°, elevation≈-5° (pointing down), z_rot=0")
    ...
    for i, (ue_pos, rotation) in enumerate(zip(dataset.rx_pos, ue_rotations)):
        ...
        expected_elevation = np.degrees(
            np.arctan2(10 - ue_pos[2], np.sqrt((0 - ue_pos[0]) ** 2 + (0 - ue_pos[1]) ** 2)),
        )

        print(
            f"  UE{i + 1}: azimuth={rotation[0]:.1f}° (exp: {expected_azimuth:.1f}°), "
...
def test_compute_channels():
    ...
    try:
        ...
        print("   ✅ BS look_at + compute_channels successful!")
        ...
        print("   ✅ UE look_at + compute_channels successful!")

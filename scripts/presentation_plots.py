...
ax.scatter(
    dataset.rx_pos[seq_idxs[::n], 0],
    dataset.rx_pos[seq_idxs[::n], 1],
    s=40,
    facecolors=(1, 0, 0, 0.5),
    edgecolors="black",
...
modify_terrain(
    terrain,
    center_xy=((max_x + min_x) / 2, (max_y + min_y) / 2),
...
if False:
    dyn_name = dm.convert(
        "RT_SOURCES/asu_campus_3p5_dyn_rd",
        overwrite=True,
        vis_scene=False,
        print_params=False,
    )

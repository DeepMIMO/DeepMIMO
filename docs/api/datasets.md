...
# Native v4 merged row/column interpretation
merged_native = raw[0, 1, 2].merge()

# Backward-compatible v3 merged row/column interpretation
merged_v3 = raw[0, 1, 2].merge(indexing="v3")

row_idxs = merged_v3.get_idxs("row", row_idxs=[0, 2, 6])
col_idxs = merged_v3.get_idxs("col", col_idxs=[0, 3, 5])
```

If you only need one RX grid but still want the v3 row/column interpretation,
merge a one-element subset explicitly:

```python
raw = dm.load("O1_60", tx_sets=[0], rx_sets=[1])
grid_v3 = raw[0,].merge(indexing="v3")
```
